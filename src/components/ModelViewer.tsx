...
interface ModelProps {
  ...
  onDimensionsCalculated?: (dim: { x: number, y: number, z: number }) => void;
}

// Helper para abstrair se é STL, OBJ ou 3MF
function Model({ url, color = "#cccccc", materialType = "pla", filename = "", forceMulticolor = false, onDimensionsCalculated }: ModelProps) {
  ...
  // Calcula BoundingBox do Modelo
  useEffect(() => {
    if (modelInstance && onDimensionsCalculated) {
      const box = new THREE.Box3().setFromObject(modelInstance instanceof THREE.Group ? modelInstance : new THREE.Mesh(modelInstance as THREE.BufferGeometry));
      const size = new THREE.Vector3();
      box.getSize(size);

      // Some buffers generate inverted / tiny boxes, safeguard to absolute values
  ...
  }, [modelInstance, onDimensionsCalculated]);
...
export default function ModelViewer({ url, color, materialType, filename = "", forceMulticolor = false, onLoaded, onDimensionsCalculated }: ModelProps & { onLoaded?: () => void }) {
