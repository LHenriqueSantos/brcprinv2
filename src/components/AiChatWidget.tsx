...
export default function AiChatWidget({ quoteId, token, mode = "portal" }: AiChatWidgetProps) {
  ...
  useEffect(() => {
    ...
    fetch("/api/config")
      .then(res => res.json())
      .then(data => {
        const welcome = data.ai_welcome_message || "Olá! Sou o assistente da brcprint. Como posso ajudar você com seu pedido ou com dúvidas sobre impressão 3D hoje?";
        setMessages([{ role: "assistant", content: welcome }]);
      })
      .catch(() => {
        setMessages([{ role: "assistant", content: "Olá! Como posso ajudar você hoje?" }]);
      });
  }, []);
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
              ...
              <div>
                <div style={{ fontWeight: 800, fontSize: "0.95rem" }}>Assistente BRC</div>
          ...
          <form onSubmit={handleSend} style={{ padding: "1rem", borderTop: "1px solid var(--border)", display: "flex", gap: "0.5rem" }}>
            <input
              ...
              placeholder={mode === "portal" ? "Pergunte sobre seu pedido ou plásticos..." : "Tire suas dúvidas sobre impressão 3D..."}
              style={{
