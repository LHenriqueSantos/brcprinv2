...
export default function ConfigPage() {
  const [form, setForm] = useState({
    ...
    melhorenvio_token: "",
    focusnfe_token: "",
    focusnfe_environment: "sandbox",
    default_nfe_type: "nfse",
    ai_system_prompt: "",
    ai_welcome_message: ""
  });
    ...
    fetch("/api/config")
      ...
      .then((d) => {
        setForm({
          ...
          melhorenvio_token: d.melhorenvio_token ?? "",
          focusnfe_token: d.focusnfe_token ?? "",
          focusnfe_environment: d.focusnfe_environment ?? "sandbox",
          default_nfe_type: d.default_nfe_type ?? "nfse",
          ai_system_prompt: d.ai_system_prompt ?? "",
          ai_welcome_message: d.ai_welcome_message ?? ""
        });
      });
    fetchPackaging();
  }, []);
  ...
  const handleTestEmail = async () => {
    if (!form.smtp_host || !form.smtp_user || !form.smtp_pass || !form.sender_email) {
      alert("Preencha e salve todos os campos de SMTP antes de testar.");
      return;
    }
    const toEmail = prompt("Digite o e-mail de destino para receber o teste:");
    if (!toEmail) return;
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok && data.success) alert("✅ E-mail de teste enviado com sucesso!");
  ...
  const handleTestWhatsapp = async () => {
    if (!form.whatsapp_api_url || !form.whatsapp_api_token) {
      alert("Preencha e salve os campos do WhatsApp (URL e Token) antes de testar.");
      return;
    }
    const phone = prompt("Digite o número do WhatsApp com DDD para receber o teste (ex: 5511999999999):");
    if (!phone) return;
    try {
      ...
      const data = await res.json();
      if (res.ok && data.success) alert("✅ Mensagem de WhatsApp enviada com sucesso!");
      else alert(`Erro: ${data.error}`);
    } catch (e: any) {
      alert("Erro de conexão ao testar WhatsApp.");
    }
  };
      ...
      <form onSubmit={handleSubmit}>
        ...
        {activeTab === "plataforma" && (
          <>
            ...
            <Section title="🤖 Assistente IA" icon="">
              {field("Mensagem de Boas-vindas (Chat Publico/Portal)", "ai_welcome_message", "A primeira frase que o robô exibe ao usuário no chat", "", "text")}
              <div style={{ marginBottom: "1.25rem" }}>
                <label className="label">Prompt do Sistema (Personalidade da IA)</label>
                <textarea
                  className="input"
                  style={{ height: "150px", resize: "vertical" }}
                  value={form.ai_system_prompt}
                  onChange={(e) => setForm({ ...form, ai_system_prompt: e.target.value })}
                  placeholder="Você é o Assistente Virtual da BRCPrint..."
                />
                <div style={{ fontSize: "0.7rem", color: "var(--muted)", marginTop: "0.5rem" }}>
                  Estas são as instruções base enviadas ao ChatGPT. O robô sempre tentará seguir estas diretrizes ao interagir com o cliente.
                </div>
              </div>
            </Section>
          </>
            ...
            <div style={{ marginTop: "1.5rem", padding: "1rem", background: "var(--surface2)", borderRadius: 8, border: "1px solid var(--border)" }}>
              ...
              {form.shipping_api_provider !== 'none' && (
                <div style={{ marginTop: "1rem" }}>
                  {field("Token de API (Cotação de Frete)", "shipping_api_token", "Insira o token para cálculo do provedor escolhido", "", "password", "")}
                </div>
              )}
              {form.shipping_api_provider === 'melhorenvio' && (
                <div style={{ marginTop: "1rem" }}>
                  {field("Token Emissão Etiquetas (Melhor Envio)", "melhorenvio_token", "Insira o token Bearer do painel do Melhor Envios para emissão automática.", "", "password", "")}
                </div>
              )}
            </div>
          </Section>
        ...
        {activeTab === "pagamentos" && (
          <>
            ...
            <Section title="Fiscal e Notas Fiscais (Focus NFe)" icon="🧾">
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
                <div>
                  <label className="label">Ambiente da API</label>
                  <select
                    className="input"
                    value={form.focusnfe_environment}
                    onChange={e => setForm({ ...form, focusnfe_environment: e.target.value })}
                  >
                    <option value="sandbox">Sandbox (Testes)</option>
                    <option value="production">Produção (NFe Real)</option>
                  </select>
                </div>
                <div>
                  <label className="label">Tipo de Nota Padrão</label>
                  <select
                    className="input"
                    value={form.default_nfe_type}
                    onChange={e => setForm({ ...form, default_nfe_type: e.target.value })}
                  >
                    <option value="nfse">NFS-e (Prestação de Serviço)</option>
                    <option value="nfe">NF-e (Venda de Produto)</option>
                  </select>
                </div>
              </div>
              {field("Focus NFe Token", "focusnfe_token", "Token de Acesso da API do Focus NFe", "", "password", "")}
            </Section>
          </>
        )}
              ...
              {form.enable_whatsapp && (
                <div style={{ marginTop: "1rem", padding: "1rem", background: "#25D36611", borderRadius: 8, border: "1px solid #25D36633" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "1rem" }}>
                    <button type="button" onClick={handleTestWhatsapp} className="btn" style={{ background: "#25D366", color: "white", fontWeight: 700 }}>
            ...
            <Section title="E-mail Padrão (SMTP)" icon="📧">
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
                ...
                <div style={{ gridColumn: "1 / -1", display: "flex", gap: "1rem", alignItems: "flex-start" }}>
                  <div style={{ flex: 1 }}>{field("E-mail Remetente (Público)", "sender_email", "Assinatura mostrada para quem recebe seu contato", "", "text", "")}</div>
                  <button type="button" onClick={handleTestEmail} className="btn btn-ghost" style={{ marginTop: "1.8rem", border: "1px solid var(--border)", whiteSpace: "nowrap" }}>
                    ✉️ Enviar E-mail Teste
                  </button>
                </div>
              </div>
            </Section>
