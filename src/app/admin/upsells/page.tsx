...
export default function UpsellsAdminPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        const res = await fetch(`/api/upsells/${editingId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload)
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || "Erro ao atualizar opcional");
        setMsg({ text: "Opcional atualizado com sucesso!", type: "success" });
      } else {
        const res = await fetch("/api/upsells", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    <>
      ...
      <div className="card" style={{ marginBottom: "2rem" }}>
        <h2 style={{ fontSize: "1.25rem", fontWeight: 700, marginBottom: "1rem" }}>
          {editingId ? "Editar Opcional" : "Adicionar Novo Opcional"}
        </h2>
        <form onSubmit={handleSubmit} style={{ display: "grid", gap: "1.5rem", gridTemplateColumns: "1fr 1fr" }}>
          ...
          <div style={{ gridColumn: "1 / -1", display: "flex", gap: "1rem" }}>
            <button type="submit" className="btn btn-primary" disabled={submitting}>
              {submitting ? "Salvando..." : editingId ? "Salvar Alterações" : "Cadastrar Opcional"}
            </button>
            {editingId && (
              <button
                type="button"
                        ...
                        <button
                          onClick={() => {
                            ...
                            setForm({
                              ...
                              charge_value: u.charge_value.toString(),
                              per_unit: !!u.per_unit,
                            });
                            window.scrollTo({ top: 0, behavior: "smooth" });
                          }}
                          className="btn"
                          style={{ padding: "0.4rem 0.8rem", color: "#3b82f6", borderColor: "#3b82f655" }}>
                          Editar
                        </button>
