...
export default function NovaCotacaoPage() {
  ...
  // Array de BoundingBox: qual URL tem quais tamanhos (x,y,z em mm padrão Slicer/Three.js)
  const [modelMetrics, setModelMetrics] = useState<Record<string, { x: number, y: number, z: number }>>({});
  // Qual URL o cliente aplicou qual Escala (Padrão 100%)
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        if (type === '3d') {
          ...
          setModelScales(prev => ({ ...prev, [upData.url]: 100 })); // Init scale 100%
  ...
  const remove3dFile = (url: string) => {
    setFileUrls(prev => prev.filter(u => u !== url));
    const m = { ...modelMetrics }; delete m[url]; setModelMetrics(m);
    const s = { ...modelScales }; delete s[url]; setModelScales(s);
  };
  ...
  const handleAutoSlice = async () => {
    ...
    try {
      ...
      // Adicionamos os dicionários de quantidades e de escalas pro slicer fazer a conta
      formData.append("quantities", JSON.stringify(
        fileUrls.reduce((acc, url) => ({ ...acc, [url.split('/').pop() || 'file.stl']: form.quantity }), {})
      ));

      formData.append("scales", JSON.stringify(
        fileUrls.reduce((acc, url) => ({ ...acc, [url.split('/').pop() || 'file.stl']: (modelScales[url] || 100) / 100 }), {})
      ));
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              {fileUrls.length > 0 ? (
                <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                  {fileUrls.map((url, i) => {
                    const metrics = modelMetrics[url];
                    const scalePct = modelScales[url] || 100;
                      ...
                      <div key={i} style={{ border: "1px solid var(--border)", borderRadius: "8px", overflow: "hidden", position: "relative" }}>
                        <div style={{ height: 400, background: "var(--surface2)" }}>
                          <ModelViewer
                            url={url}
                            color="#6c63ff"
                            materialType={form.filament_id ? filaments.find(f => f.id === Number(form.filament_id))?.type : "PLA"}
                            onDimensionsCalculated={(dim) => setModelMetrics(prev => ({ ...prev, [url]: dim }))}
                          />
                        </div>

                        {/* Barra de Ferramentas / Escala Abaixo do Modelo */}
                        <div style={{ background: "var(--surface)", borderTop: "1px solid var(--border)", padding: "0.75rem", display: "flex", justifyContent: "space-between", alignItems: "center", gap: "1rem" }}>
                          <div style={{ flex: 1, display: "flex", alignItems: "center", gap: "1rem" }}>
                            <div>
                              <label className="label" style={{ fontSize: "0.7rem", marginBottom: "0.2rem" }}>📏 Escala (%)</label>
                              <div style={{ display: "flex", alignItems: "center", background: "var(--background)", border: "1px solid var(--border)", borderRadius: "6px", overflow: "hidden" }}>
                                <input
                                  type="number"
                                  className="input"
                                  style={{ width: "80px", border: "none", borderRadius: 0, padding: "0.3rem 0.5rem", fontSize: "0.85rem", background: "transparent" }}
                                  value={scalePct}
                                  onChange={(e) => setModelScales(prev => ({ ...prev, [url]: Number(e.target.value) || 1 }))}
                                  step="1" min="1" max="10000"
                                />
                                <span style={{ padding: "0 0.5rem", color: "var(--muted)", fontSize: "0.8rem", background: "var(--surface2)", borderLeft: "1px solid var(--border)", height: "100%", display: "flex", alignItems: "center" }}>%</span>
                              </div>
                            </div>

                            {metrics ? (
                              <div style={{ fontSize: "0.75rem", color: "var(--muted)" }}>
                                <div style={{ fontWeight: 600, color: "var(--text)", marginBottom: "0.2rem" }}>Dimensões Finais:</div>
                                X: {(metrics.x * factor).toFixed(1)}mm &nbsp;|&nbsp;
                                Y: {(metrics.y * factor).toFixed(1)}mm &nbsp;|&nbsp;
                                Z: {(metrics.z * factor).toFixed(1)}mm
                              </div>
                            ) : (
                              <div style={{ fontSize: "0.75rem", color: "var(--muted)" }}>Calculando dimensões...</div>
                            )}
                          </div>

                          <button type="button" onClick={() => remove3dFile(url)} style={{ background: "rgba(239, 68, 68, 0.1)", color: "#ef4444", border: "1px solid rgba(239,68,68,0.2)", height: 32, borderRadius: "6px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.75rem", padding: "0 10px", fontWeight: 600 }}>🗑️ Remover</button>
                        </div>
                      </div>
                    );
                  })}
