...
const getPasswordResetLocales = (lang: string) => {
  const dicts: any = {
    pt: {
      subject: "Redefinição de Senha",
      greeting: "Olá",
      question: "Recebemos uma solicitação para redefinir a senha da sua conta.",
      discount: "Se você não fez essa solicitação, pode ignorar este e-mail. Caso contrário, clique no botão abaixo para criar uma nova senha. O link é válido por 1 hora.",
      cta: "Redefinir Senha",
    },
    en: {
      subject: "Password Reset",
      greeting: "Hello",
      question: "We received a request to reset the password for your account.",
      discount: "If you didn't make this request, you can safely ignore this email. Otherwise, click the button below to create a new password. The link is valid for 1 hour.",
      cta: "Reset Password",
    },
    es: {
      subject: "Restablecimiento de Contraseña",
      greeting: "Hola",
      question: "Recibimos una solicitud para restablecer la contraseña de su cuenta.",
      discount: "Si no realizó esta solicitud, puede ignorar este correo electrónico de forma segura. De lo contrario, haga clic en el botón de abajo para crear una nueva contraseña. El enlace es válido por 1 hora.",
...
export async function sendPasswordResetEmail({
  ...
  const L = getPasswordResetLocales(lang);
  const info = await transporter.sendMail({
    from: process.env.SMTP_FROM,
    to,
    subject: L.subject,
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
        <h2 style="color: #6c63ff;">${L.greeting}, ${userName}!</h2>
        <p>${L.question}</p>
        <p>${L.discount}</p>

        <div style="margin: 30px 0; text-align: center;">
          <a href="${resetUrl}" style="background-color: #6c63ff; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">
            ${L.cta}
          </a>
        </div>
        ...
        <p style="font-size: 0.75rem; color: #94a3b8; text-align: center;">
          Este é um e-mail automático enviado pelo sistema brcprint. Por favor, não responda diretamente a este e-mail.
        </p>
      </div>
    `,
  });
