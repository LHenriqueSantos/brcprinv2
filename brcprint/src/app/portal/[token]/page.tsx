      ...
      <main style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: "2.5rem" }}>
        ...
        <section>
          ...
          <div className="card" style={{ padding: "2rem", marginBottom: "2rem" }}>
            ...
            {shippingOptions.length > 0 && q.status === 'quoted' && (
          ...
          {/* ── Action Buttons (visible only while awaiting client response) ── */}
          {(q.status === 'quoted' || q.status === 'counter_offer') && (

            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              <button
                onClick={() => {
                  if (!q.client_name || !q.client_document) {
                    setShowAddressModal(true);
                  } else {
                ...
                style={{
                  ...
                  opacity: (actionLoading || q.status !== 'quoted') ? 0.6 : 1
                }}
                onMouseOver={(e) => { if (!actionLoading && q.status === 'quoted') e.currentTarget.style.transform = "translateY(-2px)" }}
                onMouseOut={(e) => { if (!actionLoading && q.status === 'quoted') e.currentTarget.style.transform = "translateY(0)" }}
              >
              ...
              <button
                ...
                onMouseOver={(e) => { if (!actionLoading && q.status === 'quoted') e.currentTarget.style.borderColor = "var(--accent)" }}
                onMouseOut={(e) => { if (!actionLoading && q.status === 'quoted') e.currentTarget.style.borderColor = "var(--border)" }}
              >
        ...
        <aside>
          <div className="card" style={{ padding: "1.5rem", position: "sticky", top: "2rem", border: "2px solid var(--accent)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
              ...
              {/* PIX Payment Section */}
              {['approved', 'awaiting_payment'].includes(q.status) && (
