    ...
    if (address_data && (action === 'approved' || action === 'identify')) {
      ...
      if (address_data.client_email) {
        ...
        if ((existing as any[]).length > 0) {
          ...
          await pool.query(
            `UPDATE clients SET name = ?, phone = ?, document = ?${extraSet} WHERE id = ?`,
            [address_data.client_name, address_data.client_phone || null, address_data.client_document || null, ...extraArgs, clientId]
          );
          if (passwordHash) accountCreated = true;
        } else {
          const [ins] = await pool.query(
            `INSERT INTO clients (name, email, phone, document, password_hash) VALUES (?, ?, ?, ?, ?)`,
