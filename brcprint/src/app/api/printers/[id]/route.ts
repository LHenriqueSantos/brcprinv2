...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const {
      ...
      api_type, ip_address, api_key
    } = body;
    await pool.query(
      `UPDATE printers SET name=?, model=?, type=?, power_watts=?, purchase_price=?, lifespan_hours=?, maintenance_reserve_pct=?, maintenance_alert_threshold=?, current_hours_printed=COALESCE(?, current_hours_printed), active=?, api_type=?, ip_address=?, api_key=? WHERE id=?`,
      [
        ...
        api_type || 'none', ip_address || null, api_key || null, id
      ]
    );
