...
export async function POST(req: Request) {
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO printers (name, model, type, power_watts, purchase_price, lifespan_hours, maintenance_reserve_pct, maintenance_alert_threshold, api_type, ip_address, api_key)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        ...
        api_type || 'none', ip_address || null, api_key || null
      ]
    );
