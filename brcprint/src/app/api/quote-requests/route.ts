...
export async function POST(req: Request) {
  try {
    ...
    const {
      title, items, notes, client_zipcode, coupon_code, upsells,
    ...
    try {
      ...
      if (config && config.enable_auto_quoting) {
        ...
        if (validFilesCount === payloadItems.length && totalBaseFinalPrice > 0) {
          ...
          const [qResult] = await pool.query(
            `INSERT INTO quotes (
                  ...
                  client_address, client_address_number, client_address_comp, client_neighborhood, client_city, client_state, client_document, client_name
                ) VALUES (?,?,?,1,1,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,30, 'quoted', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
              finalQuoteTitle, client_id, publicToken,
              totalPrintTime, totalWeightGrams, totalSetupHours, 0, totalGlobalQuantity, payloadItems[0].infill,
      ...
      console.error("Instant quoting failed:", autoErr);
    }
