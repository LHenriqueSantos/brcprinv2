...
`;
    ...
    const [filaments] = await pool.query("SELECT name, type, description FROM filaments WHERE active = 1");
    const filamentInfo = (filaments as any[]).map(f => `- ${f.name} (${f.type}): ${f.description}`).join("\n");
...
`;
    ...
    const aiText = await getAiResponse(messages, fullContext);
